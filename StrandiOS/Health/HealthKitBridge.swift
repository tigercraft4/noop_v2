...
final class HealthKitBridge: ObservableObject {
    ...
    // Water/caffeine (#949) stay read-only under `.coreRead` via `readTypes(for:)`'s sleep/workout
    // additions — they were never part of any write set pre-#653 either, so the split above doesn't
    // touch them; see `readTypes(for: .coreRead)`.
    ...
    /// Identifiers that must NEVER enter a `requestAuthorization(toShare:)` set (#1366/#1371). HealthKit
    /// reserves some quantity types to Apple Watch — `.appleSleepingWristTemperature`, the only type that
    /// fits a worn wrist skin temperature, is one — and asking to SHARE such a type does not fail softly:
    /// it raises an uncatchable ObjC `NSInvalidArgumentException` that terminates the app on launch
    /// (verified on device, iOS 27). None of the per-stage write lists below currently include it, but
    /// each stage's `writeTypes(for:)` case still filters through this denylist — the same structural
    /// defense #1371 added to the old single combined write list, kept here so a read-only id added to
    /// any ONE stage's list by mistake is dropped from that stage's ask instead of bricking launch.
    ...
    private func readTypes(for stage: ConsentStage) -> Set<HKObjectType> {
        ...
        case .coreRead:
            ...
            for id in HealthKitBridge.coreReadIds + HealthKitBridge.dietaryReadIds {
    ...
    private func writeTypes(for stage: ConsentStage) -> Set<HKSampleType> {
        ...
        case .nightlyVitalsWriteback:
            ...
            for id in HealthKitBridge.nightlyVitalsWriteIds where !HealthKitBridge.writeDenied.contains(id) {
                if let t = HKObjectType.quantityType(forIdentifier: id) { s.insert(t) }
            }
            if let sleep = HKObjectType.categoryType(forIdentifier: .sleepAnalysis) { s.insert(sleep) }
            return s
        case .heartRateWriteback:
            var s = Set<HKSampleType>()
            if !HealthKitBridge.writeDenied.contains(HealthKitBridge.heartRateWriteId),
               let t = HKObjectType.quantityType(forIdentifier: HealthKitBridge.heartRateWriteId) {
                s.insert(t)
            }
            return s
        case .workoutWriteback:
            var s = Set<HKSampleType>()
            for id in HealthKitBridge.workoutEnergyDistanceWriteIds where !HealthKitBridge.writeDenied.contains(id) {
                if let t = HKObjectType.quantityType(forIdentifier: id) { s.insert(t) }
            }
            s.insert(HKObjectType.workoutType())
            return s
    ...
    /// A stable fingerprint of the `.coreRead` stage's read types currently requested. Deliberately
    /// EXCLUDES body composition (#653 gave it its own persisted opt-in flag and its own explicit
    /// request path — see `bodyCompositionEnabled` / `requestBodyCompositionAuthorization` — so a user
    /// who has that toggled off must not be silently re-asked for it here).
    private static var readTypeSignature: String {
        (coreReadIds + dietaryReadIds).map(\.rawValue).sorted().joined(separator: ",")
    }
    ...
    /// Re-request the `.coreRead` stage when the app has STARTED reading a type it never used to (#949).
    ///
    /// HealthKit never reports read authorization, and the core read set is only requested from the
    /// connect button (`requestAuthorization()`). So a read type added in an update stays
    /// `.notDetermined` for everyone who granted access before it existed, and its queries return empty
    /// forever — indistinguishable from "you have no water in Health", and silent. Water and caffeine
    /// would have done nothing at all for every existing user, which is most of them.
    ///
    ...
    /// stored only on success, so a failed request is retried rather than silently swallowed. Scoped to
    /// `.coreRead`'s read types only (empty share set) — body composition, heart-rate write-back, and
    /// workout write-back each have their own explicit opt-in and must never be re-requested from here.
    private func requestNewReadTypesIfNeeded() async {
        ...
        do {
            try await store.requestAuthorization(toShare: writeTypes(for: .coreRead), read: readTypes(for: .coreRead))
    ...
    func refreshAuthIfPreviouslyGranted() {
        ...
        // #1024 (called from the offload write-back, in processes that were never foregrounded) needs
        // ONLY the status read below — `auth` resumed to `.authorized` so a completed backfill isn't
        // silently dropped while `auth` sits at `.unknown`. #653 needs this to keep reading the SAME
        // legacy signal it always has (nightly vitals write share status) so a pre-#653 grant still
        // resumes exactly as before staging existed.
        ...
        if granted {
            ...
            // nothing here asks HealthKit for anything new on their behalf without a tap — including when
            // this resume runs from #1024's backgrounded write-back path, where no sheet could present
            // anyway. That removes the one thing #1024 and #653 disagreed about; the status read above,
            // which #1024 actually needs, is unchanged by removing it.
        }
    }
